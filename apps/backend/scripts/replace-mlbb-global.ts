...
async function main() {
  ...
  // Fetch and validate everything before connecting or removing any packages.
  const packages = prepareMlbbCatalogue(await g2bulkAdapter.getMlbbGlobalCatalogue());
  console.log(`Validated ${packages.length} MLBB Global packages.`);
  const db = new PrismaClient({ log: [] });
  try {
    const games = await db.game.findMany({ where: { slug: { in: ["mobile-legends", "mlbb"] } } });
    ...
    const backupPath = path.join(backupDir, `mlbb-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
    ...
    const result = await db.$transaction(async (tx) => {
      const defaults = FALLBACK_GAMES.find((game) => game.slug === "mobile-legends")!;
      const game = await tx.game.upsert({
        where: { slug: "mobile-legends" },
        update: { region: "Global", isActive: true },
        create: {
          slug: "mobile-legends", name: defaults.name, category: defaults.category,
          logoUrl: defaults.logoUrl, bannerUrl: defaults.bannerUrl, region: "Global",
      ...
      for (const product of oldProducts) {
        ...
        if (orderCount > 0) {
          await tx.product.update({ where: { id: product.id }, data: { isActive: false, sku: `ARCHIVED_MLBB_${product.id}` } });
          archived++;
        } else {
      ...
      for (const entry of packages) {
        const supplierProduct = await tx.supplierProduct.upsert({
          ...
          update: { supplierGameCode: "mlbb_global", supplierProductName: entry.catalogueName, currentCost: entry.price.supplierCost, isAvailable: true, lastSyncAt: new Date() },
          create: { supplierId: supplier.id, supplierProductCode: entry.supplierProductCode, supplierGameCode: "mlbb_global", supplierProductName: entry.catalogueName, currentCost: entry.price.supplierCost },
        });
    ...
    const saved = await db.product.count({ where: { game: { slug: "mobile-legends" }, isActive: true } });
    if (saved !== packages.length) throw new Error("Imported package count verification failed.");
    console.log(`Verified ${saved} active MLBB packages in MongoDB.`);
  } finally {
    await db.$disconnect();
  }
}
...
main().catch((error) => {
  // Avoid printing connection strings or credentials from database errors.
