...
import { prepareMlbbExclusiveCatalogue } from "../lib/mlbbCatalogue";

dotenv.config({ path: path.resolve(__dirname, "../.env.local") });
dotenv.config({ path: path.resolve(__dirname, "../.env") });

const SLUG = "mobile-legends-philippines";
const SUPPLIER_GAME = "mlbb_exclusive";
...
async function main() {
  ...
  const packages = prepareMlbbExclusiveCatalogue(await g2bulkAdapter.getMlbbExclusiveCatalogue());
  console.log(`Validated ${packages.length} Mobile Legends Philippines packages.`);
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const game = await tx.game.upsert({
        ...
        update: {
          ...
          logoUrl: "https://api.g2bulk.com/images/mlbb_exclusive.png",
        ...
        create: {
          ...
          isActive: true,
          isPopular: true,
          sortOrder: sourceGame.sortOrder + 1,
        },
      });
      ...
      for (const product of existingProducts) {
        ...
        if (orderCount > 0) {
          await tx.product.update({
            where: { id: product.id },
            data: { isActive: false, sku: `ARCHIVED_MLBB_PH_${product.id}` },
          });
          archived++;
        } else {
      ...
      for (const entry of packages) {
        ...
        await tx.product.create({
          data: {
            ...entry.product,
            description: `Mobile Legends Philippines (${entry.catalogueName})`,
            gameId: game.id,
