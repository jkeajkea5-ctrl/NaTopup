  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col bg-transparent text-brand-text font-sans overflow-x-hidden">
          {!adminRoute && <Header />}
          <main className={adminRoute ? "flex-1 w-full" : "flex-1 max-w-[1440px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8"}>
            <ErrorBoundary>
              <Routes>
          ...
          {!adminRoute && <Footer />}
          {!adminRoute && <FloatingSupportButton />}
          {!adminRoute && <InstallPrompt />}
        </div>
    </>
  );
}
