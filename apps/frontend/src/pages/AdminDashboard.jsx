...
function Editor({ editor, saving, error, onClose, onSave }) {
  ...
  return <dialog ref={dialog} className="admin-dialog" onCancel={(e) => { e.preventDefault(); if (!saving) onClose(); }}>
    <form onSubmit={(e) => { e.preventDefault(); onSave({ entity: editor.entity, ...(editor.id ? { id: editor.id } : {}), data: form }); }}>
      ...
      {editor.entity === "package" && <ImageUpload label="Package image" value={form.iconUrl} disabled={saving} onChange={(iconUrl) => setForm({ ...form, iconUrl })} />}
      ...
      <fieldset disabled={saving} className="admin-fields">
        ...
        {editor.entity === "package" && <>{field("name", "Package name")}{field("amount", "Amount")}{field("iconUrl", "Image URL", "text", false)}{field("sellingPrice", "Selling price", "number")}{field("discount", "Discount", "number")}{field("sortOrder", "Display order", "number")}{toggle("isActive", "Active")}{toggle("isPopular", "Popular")}{toggle("isFeatured", "Featured")}</>}
...
export const AdminDashboard = () => {
  ...
  function edit(entity, item) {
    const fields = { game: ["name", "category", "logoUrl", "bannerUrl", "sortOrder", "isActive", "isPopular"], package: ["name", "amount", "iconUrl", "sortOrder", "isActive", "isPopular", "isFeatured", "sellingPrice", "discount"], slide: ["title", "bannerUrl", "targetUrl", "sortOrder", "isActive"], supplier: ["priority", "isEnabled"] };
    const source = entity === "package" ? { ...item, sellingPrice: item.price?.sellingPrice || 0, discount: item.price?.discount || 0 } : item;
  ...
  const packageCategories = [...new Set((data?.products || []).map((product) => product.game?.category).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  const packageGames = [...new Map((data?.products || [])
    .filter((product) => packageCategory === "ALL" || product.game?.category === packageCategory)
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-page">
      ...
      {!data ? <Empty>{loading ? "Loading NA TOPUP records…" : "No data loaded."}</Empty> : <section className="admin-panel">
        ...
        {activeTab === "packages" && <div className="admin-collection-view"><div className="admin-collection-meta"><p><strong>{filteredPackages.length}</strong> of {data.products.length} packages</p><span>{packageCategory === "ALL" ? "All categories" : packageCategory}</span></div><div className="admin-card-grid">{filteredPackages.map((product) => <article className="admin-card admin-entity-card admin-package-card" key={product.id}><div className="admin-card-title"><Picture url={product.iconUrl || product.game.logoUrl} name={product.game.name} /><div><small>{product.game.category || "Uncategorized"}</small><h3>{product.name}</h3><p>{product.game.name}</p></div></div><div className="admin-price-block"><span>Customer price</span><strong>{product.price ? money(product.price.sellingPrice - product.price.discount) : "No price"}</strong><small>{product.price ? `Cost ${money(product.price.supplierCost)} · Discount ${money(product.price.discount)}` : "Pricing record unavailable"}</small></div><div className="admin-tags"><span className={product.isActive ? "tag-active" : "tag-hidden"}>{product.isActive ? "Active" : "Hidden"}</span>{product.isPopular && <span className="tag-featured">Popular</span>}<span>{product._count.mappings} mappings</span></div><div className="admin-card-footer"><p>Display order {product.sortOrder}</p><button className="admin-btn secondary" disabled={!product.price} onClick={() => edit("package", product)}><Pencil size={15} />Edit</button></div></article>)}{!filteredPackages.length && <Empty>No packages match this category, game, and search.</Empty>}</div></div>}
