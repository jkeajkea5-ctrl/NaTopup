...
function SecurityPanel() {
  ...
  return <div className="admin-security">
    ...
    <section className="admin-card admin-invite"><div><Link2 /><h3>One-time secret URL</h3><p>The first visitor receives temporary IP access until the 30-minute link expires.</p></div><button className="admin-btn primary" onClick={createInvite}>Create 30-minute link</button>{invite && <div className="admin-copy"><input readOnly value={invite} /><button className="admin-icon-btn" onClick={() => navigator.clipboard?.writeText(invite)}><Copy size={17} /></button></div>}</section>
    <div className="admin-form-grid">
      ...
      <section className="admin-card"><h3>IP allowlist</h3><div className="admin-list">{data?.allowlist?.map((entry) => <div key={entry.id}><span><strong>{entry.ipAddress}</strong><small>{entry.label || "No label"}{entry.expiresAt ? ` · Expires ${new Date(entry.expiresAt).toLocaleString()}` : ""}</small></span>{entry.locked ? <em>Environment</em> : entry.isActive && <button className="remove" onClick={() => action({ action: "remove-ip", id: entry.id }, "IP removed.")}>Remove</button>}</div>)}{!loading && data && !data.allowlist?.length && <Empty>No approved IP addresses found.</Empty>}</div></section>
    </div>
  </div>;
}
...
export const AdminDashboard = ({ adminSession }) => {
  ...
  const [catalogSyncing, setCatalogSyncing] = useState("");
  ...
  async function syncCatalog(supplier) {
    if (packageGame === "ALL") {
      setError("Select one game before syncing its live supplier catalog.");
      return;
    }
    setCatalogSyncing(supplier); setError(""); setNotice("");
    try {
      const result = await syncAdminGameCatalog(packageGame, supplier);
      await refresh();
      setNotice(`${supplier === "G2BULK" ? "G2Bulk" : "Vizo"} · ${result.game}: ${result.added} added, ${result.updated} updated, ${result.deleted} deleted, ${result.archived} archived.`);
    } catch (err) {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-page">
      ...
      {!data ? <Empty>{loading ? "Loading NA TOPUP records…" : "No data loaded."}</Empty> : <section className="admin-panel">
        ...
        {activeTab === "profit" && <>
          {profitError && <p className="admin-alert error">{profitError}</p>}
          <div className="admin-summary admin-profit-summary">
            <article><WalletCards /><span><small>Revenue</small><strong>{money(metrics.revenue)}</strong><em>Paid orders</em></span></article>
            <article><Boxes /><span><small>Supplier costs</small><strong>{money(metrics.cost)}</strong><em>Order cost snapshots</em></span></article>
            <article><WalletCards /><span><small>Gross profit</small><strong>{money((metrics.revenue || 0) - (metrics.cost || 0))}</strong><em>{metrics.revenue ? (((metrics.revenue - metrics.cost) / metrics.revenue) * 100).toFixed(1) : 0}% margin</em></span></article>
            <article><WalletCards /><span><small>Today&apos;s profit</small><strong>{money(metrics.daily?.profit)}</strong><em>{metrics.daily?.paidOrders || 0} paid order{metrics.daily?.paidOrders === 1 ? "" : "s"} today · Cambodia</em></span></article>
          </div>
          <p className="admin-hint inset">Daily profit and supplier wallets refresh automatically every 30 seconds{profitUpdatedAt ? ` · Last checked ${new Date(profitUpdatedAt).toLocaleTimeString()}` : ""}.</p>
          <div className="admin-card-grid">{liveBalances?.map((wallet) => <article className="admin-card admin-wallet-card" key={wallet.supplier}><div className="admin-card-title"><WalletCards /><div><h3>{wallet.supplier}</h3><p>{wallet.accountName || "Supplier account"}</p></div></div><strong className="admin-amount">{wallet.isHealthy ? `${Number(wallet.balance).toFixed(2)} ${wallet.currency}` : "Unavailable"}</strong><div className="admin-tags"><span className={wallet.isHealthy ? "wallet-online" : "wallet-offline"}>{wallet.isHealthy ? "Live" : "Connection failed"}</span></div><p>{wallet.isHealthy ? `Checked ${new Date(wallet.lastChecked).toLocaleString()}` : wallet.errorMessage || "Could not retrieve the live balance."}</p></article>)}{liveBalances && !liveBalances.length && <Empty>No supplier wallets are configured.</Empty>}{!liveBalances && <Empty>{profitLoading ? "Loading live supplier balances…" : "Live balances have not loaded."}</Empty>}</div>
        </>}
